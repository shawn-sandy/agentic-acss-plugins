...
- **`/setup` writes a `.browserslistrc` with a [Baseline](https://web.dev/baseline) target** (new Step 6.5). The file contains one query — `baseline widely available` — which [Browserslist supports natively](https://web.dev/blog/browserslist-supports-baseline) and which every tool already reading Browserslist inherits for free: autoprefixer, Lightning CSS, esbuild targets, `stylelint-browser-compat`. That target matches what the generated themes and components are written against (`@layer`, `oklch()`, `color-mix()`, `:has()`). **Skipped when the project already has a `.browserslistrc` or a `package.json` `browserslist` key** — overwriting an existing browser target would silently change what the user's build compiles for. Follows the existing idempotency contract: created or kept, never forced.
