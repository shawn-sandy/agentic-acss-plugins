...
- **`scripts/generate_color_scale.py`** — generator/validator script that produces the 10-step scale. Accepts `<hex-color> [--name=<name>] [--format=css|json|both]` (default format: `both`). Validates `--name` as a kebab-case identifier and rejects unknown flags (exit 2). JSON output includes `seed_oklch` for transparency; `--format=both` emits the JSON section followed by a blank line and the CSS block. CSS output follows the `var(--x, <fallback>)` convention. Reuses `_oklch.py` for all OKLCH ↔ sRGB math. Exit 0/2 per generator contract.
