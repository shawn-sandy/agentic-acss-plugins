...
List available components or inspect one without writing any files. Read-only — useful for discovering names, props, CSS variables, and dependencies before running `/kit-add`. The full reference (signature, examples, output shape) is in [`docs/prompt-book.md`](docs/prompt-book.md).
...
Print a copy-paste catalogue of natural-language prompts for every shipped slash command. Useful when you'd rather describe what you want than remember command syntax.
