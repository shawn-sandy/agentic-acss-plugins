...
props:
  ...
  interactive:
    type: boolean
    maps-to: "data-card=interactive"
    a11y: "adds role=button, tabIndex=0, Enter/Space activation (WCAG 2.1.1)"
  onClick:
    type: function
    a11y: "required when interactive=true"
  classes:
    type: string
  styles:
    type: CSSProperties
...
variants:
  interactive: { maps-to: "data-card=interactive" }
behavior: interactive-card-activation
a11y: [1.4.3, 1.4.11, 2.1.1, 2.4.7, 2.5.8, 4.1.2]
targets: [react, html, astro, angular, vue, svelte, web-component]
---

# Component: Card

> **Neutral COMPONENT.md** for the acss-kit `card`. The framework-agnostic
...
> `reference.md`). `/kit-add card` reads this file: `## Styles` → `card.scss`,
...
> npm `6.6.0`). Card uses the compound pattern (`Card`, `Card.Title`,
> `Card.Content`, `Card.Footer`) in a single `card.tsx` — same structure as
> upstream. Generated locally via `Object.assign(CardRoot, { Title, Content, Footer })`.

## Overview

A flexible container for grouping related content. Uses the compound component
pattern: `Card`, `Card.Title`, `Card.Content`, `Card.Footer` — all in a single
generated file. Supports an interactive (clickable) variant with keyboard
support.
...
A static card is plain markup — no JS required. The interactive variant adds
`data-card="interactive"`, `role="button"`, `tabindex="0"`, and `aria-labelledby`
so it's reachable by keyboard and announced as a button by assistive tech. Pair
the interactive variant with `card.js` to wire keyboard activation.

## Props

| Prop | Type | Required | Surfaces as |
|---|---|---|---|
| `as` | `React.ElementType` (default `div`) | no | host element |
| `interactive` | boolean | no | `data-card="interactive"` + `role="button"` + `tabIndex` |
| `onClick` | `() => void` | when interactive | click / Enter / Space handler |
| `classes` | string | no | merged into `card` class |
| `styles` | `React.CSSProperties` | no | inline styles |

Sub-components — `Card.Title` (`as` default `h3`), `Card.Content` (`as` default
`article`), `Card.Footer` (`as` default `div`) — each accept `as`, `children`,
and the native props of their default element.
...
**`interactive-card-activation`** — when `interactive` is set, the whole card
becomes a single click/keyboard target. It receives `role="button"`,
`tabIndex={0}`, and a `data-card="interactive"` hook for SCSS. The card responds
to Enter and Space via `onKeyDown` (with `e.preventDefault()` on Space to block
the page scroll) and invokes `onClick`. Static (non-interactive) cards carry no
interactive semantics and need no JS.
...
The custom-event pattern lets the user wire whatever logic they want without
modifying the generated module — listen for `card:activate` on a parent element
and route to the right handler. A generator realizes this spec idiomatically per
target; the `react` adapter below ships the inlined `onKeyDown` activation in the
root component.
...
<!-- static card -->
<div class="card">
  <h3 class="card-title" id="card-1-title">Product Name</h3>
  <article class="card-content">
    <p>Product description here...</p>
  </article>
  <div class="card-footer">
    <button type="button" class="btn">Buy Now — $29.99</button>
  </div>
</div>
...
>
  <h3 class="card-title" id="card-2-title">5 Tips for Better Code</h3>
  <article class="card-content">
...
`generation: { export: Card (compound: Card.Title, Card.Content, Card.Footer), file: card.tsx, scss: card.scss, imports: "UI from '../ui'", dependencies: [] }`

The React adapter is the canonical TSX projection. All sub-components are
generated in the same `card.tsx` file — no separate files. `/kit-add card
--target=react` emits the assembled file: the Props Interface, the Key Pattern,
and the TSX Template below.
