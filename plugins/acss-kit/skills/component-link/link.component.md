...
props:
  ...
  target:
    type: string
    a11y: "`_blank` triggers automatic `rel=\"noopener noreferrer\"`"
  rel:
    type: string
    maps-to: "rel"
  prefetch:
  ...
  styles:
    type: object
slots: [children]
a11y: [1.4.1, 1.4.3, 1.4.11, 2.1.1, 2.4.4, 2.4.7, 4.1.2]
targets: [react, html, astro, angular, vue, svelte, web-component]
---

# Component: Link

> **Neutral COMPONENT.md** for the acss-kit `link`. The framework-agnostic
...
> the upstream `IconLink` and `LinkButton` compound subcomponents are dropped from
> the default vendoring — they're trivial enough to add inline if needed (each is
> ~5 lines), and most consumers want one or the other but rarely both.

## Overview

A semantic anchor wrapper with automatic security defaults for external links.
Renders a native `<a>`. When `target="_blank"` is set, the component automatically
merges `rel="noopener noreferrer"` with any user-provided rel tokens, preventing
window.opener exploitation and referrer-header leakage. Supports an optional
...
<!-- variant: external (target="_blank" → automatic rel="noopener noreferrer") -->
<a href="https://example.com" target="_blank" rel="noopener noreferrer">
  <!-- slot: children -->
</a>

<!-- variant: external + user rel (merged with security defaults) -->
<a href="https://partner.com" target="_blank" rel="sponsored noopener noreferrer">
  <!-- slot: children -->
</a>

<!-- variant: icon-only (accessible name required) -->
<a href="/settings" aria-label="Settings">
  <!-- slot: icon -->
</a>

<!-- variant: button-style link (visually a button; semantically still an anchor) -->
<a href="/signup" data-btn="block">
  <!-- slot: children -->
</a>
```

The host element is `<a>`. When `target="_blank"`, security tokens
(`noopener noreferrer`) are merged into `rel`; button-style links surface via the
`data-btn` attribute.
...
|---|---|---|---|
| `href` | string | yes | `href` attribute |
| `target` | string (e.g. `_blank`) | no | `target` attribute (triggers rel merge) |
| `rel` | string | no | `rel` (merged with security defaults when `target="_blank"`) |
| `prefetch` | boolean | no | adds `prefetch` rel token (default `false`) |
| `btnStyle` | string | no | `data-btn` attribute |
| `styles` | object | no | inline styles |
...
<a href="/signup" data-btn="block">Get started</a>
```

## Target: react

`generation: { export: Link, file: link.tsx, scss: link.scss, imports: "UI from '../ui'", dependencies: [] }`

The React adapter is the canonical TSX projection — the security-merge `rel`
computation runs in a `useMemo`. `/kit-add link --target=react` emits the
