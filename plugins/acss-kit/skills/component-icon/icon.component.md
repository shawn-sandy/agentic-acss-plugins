...
props:
  name:
    values: [info, success, warning, error, close, chevron-down, chevron-right, check, external-link]
    required: true
  size:
    type: number
    default: 16
    maps-to: "width/height attributes"
  color:
    type: string
    default: currentColor
    maps-to: "fill attribute"
  aria-hidden:
    type: boolean
    default: true
    a11y: "decorative — screen readers skip the SVG (WCAG 1.1.1)"
  aria-label:
    type: string
    a11y: "informative — adds accessible name + role=img; drops aria-hidden (WCAG 1.1.1)"
slots: []
variants: {}
a11y: [1.1.1, 1.4.1, 1.4.11]
targets: [react, html, astro, angular, vue, svelte, web-component]
---

# Component: Icon

> **Neutral COMPONENT.md** for the acss-kit `icon`. The framework-agnostic
...
> `reference.md`). `/kit-add icon` reads this file: `## Target: react` →
> `icon.tsx`. Icon has no SCSS file — it styles via props (`size`, `color`) and
...
> vendored Icon is a `name`-prop dispatcher with a small built-in SVG set (info,
> success, warning, error, close, chevron-down, chevron-right, check,
> external-link). Self-contained, much smaller, and covers the icons the rest of
> the kit-builder components depend on. If a project needs the larger fpkit icon
> library, it can extend the `ICON_SVGS` map locally.

## Overview

A lightweight inline-SVG icon component. Each icon is rendered as a `<svg>`
inside the Icon component, sized via the `size` prop, colored via `color`
(default `currentColor`), and accessible-named via `aria-hidden` (decorative) or
`aria-label` (standalone).

## Semantic Structure

```html
<!-- variant: decorative (default — screen readers skip it) -->
<svg
  width="16"
  height="16"
  viewBox="0 0 24 24"
  ...
  aria-hidden="true"
>
  <!-- slot: glyph (one of the built-in ICON_PATHS) -->
</svg>

<!-- variant: informative / standalone (accessible name, role=img, no aria-hidden) -->
<svg
  width="24"
  height="24"
  viewBox="0 0 24 24"
...
The host element is `<svg>`. `size` surfaces as the `width`/`height` attributes
(pixels) and `color` as the `fill` attribute. When `aria-label` is present the
component swaps `aria-hidden` for `role="img"` so the label is announced (WCAG
1.1.1); otherwise the icon is decorative and hidden from assistive tech.

## Props

| Prop | Values | Required | Surfaces as |
|---|---|---|---|
| `name` | `info` \| `success` \| `warning` \| `error` \| `close` \| `chevron-down` \| `chevron-right` \| `check` \| `external-link` | yes | selects the built-in glyph |
| `size` | number (default `16`) | no | `width` / `height` attributes (px) |
| `color` | string (default `currentColor`) | no | `fill` attribute |
| `aria-hidden` | boolean (default `true`) | no | `aria-hidden` attribute (decorative) |
| `aria-label` | string | no | accessible name + `role="img"` (drops `aria-hidden`) |
...
<!-- Standalone — accessible name + role=img -->
<svg width="24" height="24" viewBox="0 0 24 24" fill="var(--color-warning)" aria-label="Warning" role="img">
  <!-- warning glyph -->
</svg>
```
...
`generation: { export: Icon, file: icon.tsx, scss: (none), imports: "React (only)", dependencies: [] }`

The React adapter is the canonical TSX projection — Icon is a self-contained
`name`-prop dispatcher over a built-in `ICON_PATHS` map, with no shared
hook/util tree and no SCSS file. `/kit-add icon --target=react` emits the
assembled file: the Props Interface and the TSX Template below.
