...
props:
  ...
  severity:
    ...
    maps-to: "class alert--<severity>; role + aria-live"
  title:
    type: string
  dismissible:
    type: boolean
    maps-to: "data-alert-dismissible; .alert-dismiss button"
    a11y: "Escape dismisses; button has aria-label"
  onDismiss:
    type: function
  variant:
    values: [outlined, filled, soft]
    maps-to: "class alert--<variant>"
  autoHideDuration:
    type: number
    maps-to: "data-alert-auto-hide"
    a11y: "pause-on-hover compensates timing (WCAG 2.2.1)"
  pauseOnHover:
    type: boolean
    default: true
    maps-to: "data-alert-pause-on-hover"
  hideIcon:
    type: boolean
  titleLevel:
    values: [2, 3, 4, 5, 6]
    maps-to: "title element h2-h6 (else <strong>)"
  actions:
    type: node
slots: [children, icon, title, message, actions]
variants:
  outlined: { maps-to: "class alert--outlined" }
  filled:   { maps-to: "class alert--filled" }
  soft:     { maps-to: "class alert--soft" }
behavior: alert-dismiss-autohide
a11y: [1.4.3, 1.4.11, 2.1.1, 2.2.1, 2.4.7, 4.1.2, 4.1.3]
targets: [react, html, astro, angular, vue, svelte, web-component]
---

# Component: Alert

> **Neutral COMPONENT.md** for the acss-kit `alert`. The framework-agnostic
...
> `reference.md`). `/kit-add alert` reads this file: `## Styles` → `alert.scss`,
> `## Target: react` → `alert.tsx`.
>
> **Verified against fpkit source:** `@fpkit/acss@6.5.0` (closest tagged ref to
> npm `6.6.0`). Generated Alert inlines the severity icon SVGs and
> `useAlertBehavior` hook for self-contained vendoring (no Icon component
> dependency, no separate hooks file).
...
The host element is `<div>` carrying `role="status"` (polite) for info/success/warning, or `role="alert"` (assertive) for error. Severity surfaces as the `alert--<severity>` class, the visual variant as `alert--<variant>`. The `.alert-sr-only` span replaces the React `SR_ONLY` inline style — its CSS lives in `alert.scss` (`position: absolute; width: 1px; height: 1px; clip: rect(0, 0, 0, 0); overflow: hidden;`). For severity icons, paste the inline SVG from the **Key Pattern: Severity Icon SVGs** section into the `<!-- slot: icon -->` placeholder. `data-alert-dismissible` and `data-alert-auto-hide` are read by `alert.js` at runtime.

## Props

| Prop | Values | Required | Surfaces as |
|---|---|---|---|
| `open` | boolean | yes | render gate (mounts/unmounts the alert) |
| `severity` | `default` \| `info` \| `success` \| `warning` \| `error` | no | `alert--<severity>` class + `role`/`aria-live` |
| `title` | string | no | `.alert-title` content |
| `dismissible` | boolean | no | `data-alert-dismissible` + `.alert-dismiss` button |
| `onDismiss` | function | no | callback fired on dismissal |
| `variant` | `outlined` \| `filled` \| `soft` | no | `alert--<variant>` class |
| `autoHideDuration` | number (ms) | no | `data-alert-auto-hide` |
| `pauseOnHover` | boolean (default true) | no | `data-alert-pause-on-hover` |
| `hideIcon` | boolean | no | omits `.alert-icon` |
| `titleLevel` | `2`…`6` | no | renders title as `<h2>`–`<h6>` (else `<strong>`) |
| `actions` | node | no | `.alert-actions` content |
...
**`alert-dismiss-autohide`** — when `dismissible` is set, the alert renders a
`.alert-dismiss` button that hides the alert on click (and removes it from the
DOM after the opacity transition); pressing `Escape` while a dismissible alert
is visible also dismisses it. When `autoHideDuration` (ms) is set, a timer
auto-dismisses the alert; `pauseOnHover` (default true) pauses the timer while
the user hovers or keyboard-focuses the alert so reading is not interrupted
(WCAG 2.2.1). Dismissal dispatches an `alert:dismiss` event and invokes
`onDismiss`.
...
<!-- error with title + dismiss -->
<div class="alert alert--error alert--outlined" role="alert" aria-live="assertive" data-alert-dismissible="true">
  <span class="alert-sr-only">error: </span>
  <div class="alert-content">
    <strong class="alert-title">Payment failed</strong>
    <div class="alert-message">Please check your card details and try again.</div>
  </div>
  <button type="button" class="alert-dismiss" aria-label="Dismiss alert">×</button>
</div>

<!-- success, auto-dismiss after 5s -->
<div class="alert alert--success alert--outlined" role="status" aria-live="polite" data-alert-auto-hide="5000">
  <span class="alert-sr-only">success: </span>
  <div class="alert-content">
...
`generation: { export: Alert, file: alert.tsx, scss: alert.scss, imports: "UI from '../ui'", dependencies: [] }`

The React adapter is the canonical TSX projection — the severity icon SVGs and
the `useAlertBehavior` hook are inlined into `alert.tsx` (no separate Icon
component, no shared hooks file). `/kit-add alert --target=react` emits the
assembled file: the Props Interface, the inlined Key Patterns, and the TSX
Template below.
