...
props:
  isBlock:
    type: boolean
    a11y: "vertical (block) layout on the list instead of horizontal"
  aria-label:
    type: string
    a11y: "required when multiple <nav> elements exist on the page (WCAG 2.4.8)"
slots: [children]
variants:
  block: { maps-to: "nav-list--block / data-list~=block" }
a11y: [2.4.7, 2.4.8, 4.1.2]
targets: [react, html, astro, angular, vue, svelte, web-component]
---

# Component: Nav

> **Neutral COMPONENT.md** for the acss-kit `nav` (compound: `Nav`, `Nav.List`,
> `Nav.Item`). The framework-agnostic source of truth lives in the `##` body
> below; the React projection is the `## Target: react` adapter at the end
> (byte-aligned with the legacy `reference.md`). `/kit-add nav` reads this file:
> `## Styles` → `nav.scss`, `## Target: react` → `nav.tsx`.
>
> **Legacy shape:** nav predates the `## TSX Template` convention — its React
> projection is described by `## Key Pattern: Compound Component Assembly` plus
> `## Props Interface`, with no `## TSX Template` block. Extraction therefore
> mirrors `reference.md` exactly (no golden).
...
<nav class="nav" aria-label="Main navigation">
  <ul class="nav-list" data-list="unstyled">
    <li class="nav-item"><a href="/" aria-current="page"><!-- slot: children --></a></li>
    <li class="nav-item"><a href="/about"><!-- slot: children --></a></li>
  </ul>
</nav>

<!-- variant: vertical (block) -->
<nav class="nav" aria-label="Sidebar navigation">
  <ul class="nav-list nav-list--block" data-list="unstyled block">
    <li class="nav-item"><a href="/dashboard"><!-- slot: children --></a></li>
  </ul>
</nav>
```

The host element is `<nav>` — a landmark. The list uses `<ul>` + `<li>`; the
vertical layout surfaces as the `nav-list--block` class (and a `data-list~="block"`
token). `aria-current="page"` marks the active link.

## Props

| Prop | Values | Required | Surfaces as |
|---|---|---|---|
| `aria-label` | string | conditional | `aria-label` attribute (required with multiple navs — WCAG 2.4.8) |
| `isBlock` (on `Nav.List`) | boolean | no | `nav-list--block` class + `data-list~="block"` |
| `classes` | string | no | appended to the host class list |
| `styles` | object | no | inline styles |
| `id` (on `Nav.Item`) | string | no | `id` attribute |
...
`generation: { export: Nav (compound: Nav.List, Nav.Item), file: nav.tsx, scss: nav.scss, imports: "UI from '../ui'", dependencies: [] }`

The React adapter is the canonical TSX projection. nav is **legacy-shape**: it has
no `## TSX Template` block — the generated `nav.tsx` is assembled from the
`## Props Interface` and `## Key Pattern: Compound Component Assembly` below.
