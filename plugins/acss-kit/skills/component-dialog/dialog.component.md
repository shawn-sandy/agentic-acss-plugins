...
variants:
  full:    { maps-to: "header + body + footer" }
  minimal: { maps-to: "body only" }
behavior: native-dialog-modal
a11y: [1.4.3, 2.1.1, 2.1.2, 2.4.3, 2.4.7, 4.1.2]
targets: [react, html, astro, angular, vue, svelte, web-component]
---

# Component: Dialog

> **Neutral COMPONENT.md** for the acss-kit `dialog`. The framework-agnostic
...
> `reference.md`). `/kit-add dialog` reads this file: `## Styles` → `dialog.scss`,
> `## Target: react` → `dialog.tsx`.
>
> **Native `<dialog>`:** the element provides focus trapping, scrim, Escape-to-close,
> and `aria-modal` behavior for free via `showModal()` — no `focus-trap` package.
> Intentional divergence from fpkit: the generated Dialog does NOT depend on
> `icon-button`; the close button is a `Button` variant rendering a `×` glyph,
> which simplifies the dependency tree.

## Overview

A modal dialog using the native HTML `<dialog>` element. The native element
provides focus trapping automatically via `showModal()` — no npm packages needed.
Supports a header/body/footer structure, dismissable via Escape key (built-in),
backdrop click, and a close button.
...
The host element is the native `<dialog>`. Opened with `showModal()`, the browser
provides implicit `role="dialog"`, `aria-modal="true"`, focus trapping, the
`::backdrop` scrim, and Escape-to-close. `aria-labelledby` points at the title's
id so screen readers announce the dialog by name (WCAG 4.1.2). The dialog must
carry an `id` so an external `[data-dialog-open="<id>"]` trigger can find it.

## Props

| Prop | Values | Required | Surfaces as |
|---|---|---|---|
| `dialogRef` | ref | yes | `ref` on the dialog — required for `showModal()`/`close()` |
| `openOnMount` | boolean | no | native `open` attribute |
| `onClose` | function | no | callback fired when the dialog closes |
| `title` | string | no | `<h2 class="dialog-title">` + `aria-labelledby` |
| `description` | string | no | `<p class="dialog-description">` + `aria-describedby` |
| `showCloseButton` | boolean (default `true`) | no | renders the header close button |
| `footer` | node | no | `<div class="dialog-footer">` slot |
| `children` | node | no | `<div class="dialog-body">` slot |
| `classes` | string | no | appended to the `dialog` class |
| `styles` | object | no | inline styles on the dialog |
...
**`native-dialog-modal`** — the dialog opens modally and closes on three
triggers, all leaning on the native `<dialog>` element rather than custom JS:

- **Open** — an external trigger with `[data-dialog-open="<dialog-id>"]` calls
  `dialog.showModal()`. `showModal()` traps focus inside the dialog, renders the
  `::backdrop` scrim, and applies implicit `role="dialog"` + `aria-modal="true"`.
- **Close (Escape)** — the browser fires the native `cancel` event on Escape and
  closes the dialog. No JavaScript keybinding needed.
- **Close (close button)** — an internal `[data-dialog-close]` element calls
  `dialog.close()`.
- **Close (backdrop click)** — a click whose target is the `<dialog>` element
  itself (not a descendant) calls `dialog.close()`. Clicks inside the body do not
  dismiss.
- On any close, focus returns to the element that called `showModal()` — native
  browser behavior. The init is idempotent: a `data-acss-dialog-init` sentinel
  prevents double-binding when `init()` runs twice on the same root.
...
`CSS.escape` guards against ids that contain special characters. The module relies
on the native `<dialog>` element's behavior — there is no custom focus trap or
scrim; the browser provides them. A generator realizes this spec idiomatically per
target (React ref + `showModal()`/`close()`, Vue composable, Svelte action,
Angular directive); the `react` adapter below ships the canonical `dialogRef`
projection.
...
```html
<!-- An external trigger opens the dialog by matching its id (wired by dialog.js). -->
...
<dialog id="dialog-1" class="dialog" aria-labelledby="dialog-1-title">
  <div class="dialog-header">
    <div class="dialog-header-content">
      <h2 id="dialog-1-title" class="dialog-title">Confirm Action</h2>
      <p class="dialog-description">This action cannot be undone.</p>
    </div>
    <button type="button" class="btn dialog-close" data-style="icon" aria-label="Close dialog" data-dialog-close>×</button>
  </div>
  <div class="dialog-body">
    <p>Are you sure you want to proceed?</p>
  </div>
  <div class="dialog-footer">
    <button type="button" class="btn" data-style="outline" data-dialog-close>Cancel</button>
...
`generation: { export: Dialog (+ DialogHeader, DialogBody, DialogFooter), file: dialog.tsx, scss: dialog.scss, imports: "UI from '../ui', Button from '../button/button'", dependencies: [button] }`

The React adapter is the canonical TSX projection — `Dialog` is a compound
component (`Dialog.Header`, `Dialog.Body`, `Dialog.Footer`) assembled with
`Object.assign`, driven by a `dialogRef` the parent uses to call `showModal()` /
`close()`. Unlike the fpkit source the generated Dialog does NOT depend on
`icon-button`; the close button is a `Button` variant rendering a `×` glyph.
`/kit-add dialog --target=react` emits the assembled file: the Props Interface(s),
the Key Patterns, and the TSX Template below.
