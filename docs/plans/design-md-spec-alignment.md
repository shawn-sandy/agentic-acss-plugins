...
> decisions are resolved (see [Locked decisions](#locked-decisions)); execution
> is split across two plans (links in [Next step](#next-step)).

## TL;DR

Google's **DESIGN.md** is a token/visual-identity format (YAML front-matter +
prose) that agents consume to build UIs. It overlaps with **our `styles`/theme
layer**, not our component `reference.md`. The proposal is therefore two
complementary pieces:

- **A — consume DESIGN.md.** Add it as an input to our theme pipeline:
  `npx … export css-tailwind` → `design_md_to_tokens.py` (remap Material-3 names
  to our roles, synthesize the roles M3 omits via OKLCH) → our theme CSS, gated
  by `validate_theme.py`. Reaching full parity means giving our token layer a
  home for **typography, spacing, and rounded** (it's colors-only today), which
  ripples through ~163 declaration sites in the 15 components.
- **B — publish `COMPONENT.md`.** Formalize our 9-section component
  `reference.md` shape into a versioned, portable spec (in `style-agent`) that
  references DESIGN.md tokens via `{token.path}` — yielding a **two-file design
  system**: DESIGN.md owns tokens, COMPONENT.md owns component implementation.
...
|---|---|
| [Context](#context) · [Core finding](#the-core-finding) | What DESIGN.md is; how it relates to us |
| [Architecture at a glance](#architecture-at-a-glance) | The end-to-end flow in one diagram |
| [Locked decisions](#locked-decisions) | The 6 settled decisions |
| [Workstream A](#workstream-a--let-our-components-consume-a-designmd) | Consuming DESIGN.md (mapping, parity, sweep) |
| [Round-trip & export](#round-trip--export-us--designmd) | Export *out* to DESIGN.md/DTCG/Tailwind |
| [Workstream B](#workstream-b--publish-a-sibling-spec-for-our-component-markdown) | The `COMPONENT.md` spec |
| [Workstream C](#workstream-c--tooling--integration-surface) | Tooling, parse route, MCP |
| [How projects benefit](#how-projects-take-advantage-of-designmd) · [Risks](#risks--tensions) | Value + honest tensions |
| [Definition of done](#definition-of-done) · [Non-goals](#non-goals) | Scope boundaries |
| [Open questions](#open-questions-for-review) · [Next step](#next-step) | What's left; execution links |
| [Appendices A–G](#appendix-a--m3--our-roles-translation-table-draft) | Grounded artifacts (tables, examples, contracts) |
...
## Round-trip & export (us → DESIGN.md)

Workstream A covers the *inbound* direction. The upstream CLI is **export-only
and cannot import**, so the *outbound* direction is ours to build — and it's what
makes the integration bidirectional rather than a one-way consumer. Two distinct
exports:

### 1. Our theme → DESIGN.md (`tokens_to_design_md.py`)

The inverse of the adapter, and the import-*into*-DESIGN.md path upstream lacks.
It reads our generated theme CSS (`light.css` + `space-radius.css` +
`typography.css`, via `css_to_tokens.py`), applies the **inverse of Appendix A**
(our roles → DESIGN.md token names), and emits a DESIGN.md: deterministic YAML
front-matter (`colors`/`typography`/`spacing`/`rounded`, plus a `components`
block synthesized from per-component CSS vars) and a **prose skeleton** with the
canonical `##` sections (Overview, Colors, …) pre-filled with token summaries and
`TODO` rationale placeholders the `/design-export` flow can optionally enrich.

This unlocks the workflow the import path can't: **author a brand in our pipeline**
(seed hex → OKLCH → contrast-gated theme) **and publish a DESIGN.md** that other
agents, tools, or designers consume — turning our generator into a DESIGN.md
*producer*, not just a consumer.

### 2. DESIGN.md → DTCG / Tailwind (free via the upstream CLI)

Once a project has a DESIGN.md (authored, or generated by #1),
`npx @google/design.md export --format dtcg|json-tailwind|css-tailwind` yields
W3C Design Tokens and Tailwind configs **for free**. `/design-export
[--format=design-md|dtcg|tailwind]` wraps both: `design-md` calls
`tokens_to_design_md.py`; the others shell the upstream CLI.

### Fidelity caveat (stated honestly)

The round-trip is **value-preserving, not byte-identical**. On import we
*collapse* the M3 surface/on-pairs into our 18 roles and *synthesize* the roles
M3 omits; on export we emit our 18 roles, not the full M3 ladder. So
`DESIGN.md → ours → DESIGN.md` preserves the values of the roles we model but
won't reproduce dropped M3 tokens (`surface-tint`, `*-container` pairs,
`*-fixed`). This is acceptable — DESIGN.md accepts any color token names — but it
means we are a **semantic round-trip, not a lossless one**, and the docs should
say so.
...
- **Round-trip is semantic, not lossless.** The CSS → DESIGN.md export path
  (now in [Round-trip & export](#round-trip--export-us--designmd)) preserves the
  values of the roles we model but drops M3 tokens we don't (the collapse is
  one-way). Acceptable, but must be documented so nobody expects byte-identity.

## Definition of done

The DESIGN.md alignment is complete when:

1. A project can run **`/theme-from-design DESIGN.md`** and get a full,
   contrast-gated theme — colors + typography + spacing + rounded — with
   synthesized roles reported.
2. The **15 components consume tokens** (`var(--space/radius/font-*)`), so a
   DESIGN.md or `/theme-*` change actually reskins them; no bare rem/px remain in
   swept properties; the `--color-primary-dark` and `alert` debts are fixed.
3. **`COMPONENT.md` is published** in `style-agent` with a worked example, and
   the 15 `reference.md` docs are noted as conforming to it.
4. The **round-trip works**: `/design-export` emits a valid DESIGN.md (lint-clean)
   and DTCG/Tailwind; a Figma file can seed a theme via the bridge.
5. **Validation is layered**: `validate_design_md.py` (input) + `validate_theme.py`
   (contrast) + `validate_tokens.py` (scale/units) all gate, and `tests/run.sh`
   carries a DESIGN.md round-trip step.
6. Docs are truthful: READMEs/CHANGELOGs/marketplace updated; the `npx`
...
- **Replacing our theme format with DESIGN.md.** Our CSS-custom-property theme
  files remain the authoring surface; DESIGN.md is an interchange format we
  consume and emit.
- **Making our internal token JSON *be* DTCG.** We target DTCG only on export
  (free via the CLI); we don't couple our schema to the evolving draft.
- **A lossless round-trip.** We are a semantic round-trip (values of the roles we
  model), not byte-for-byte (see fidelity caveat).
- **Multi-framework theme generation in `style-agent`.** Blocked by vocabulary
  coupling + the plugin self-containment constraint (see
  [`plugins-refactoring.md`](plugins-refactoring.md)); revisit only on demand.
- **Component-level DESIGN.md tokens.** We map DESIGN.md's *primitive* groups,
  not its freeform `components.*` entries (which don't align 1:1 with our kit).
- **Tracking DESIGN.md past `alpha` automatically.** We pin a commit SHA and
  isolate all DESIGN.md knowledge in the adapter; spec drift is a manual bump.
