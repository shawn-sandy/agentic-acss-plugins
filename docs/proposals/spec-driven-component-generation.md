...
## New skill vs. refactor — recommendation
...
- **Projection fidelity for non-React targets.** Behavior/reactivity is the lossy
  ...
  components (img, link, list) project trivially.
...
- **Verification gap.** `verify_integration.py` is React/Sass-aware today;
  non-React targets need their own light integration check (or a documented
  manual step) until extended.

## Decisions to resolve

1. **Refactor vs. new command** — evolve `/kit-add` (recommended) or add a
   sibling `/kit-build` and deprecate `/kit-add` later?
2. **Default target** — infer from `detect_stack.py` (recommended) or require an
   explicit `--target`?
3. **DESIGN.md discovery** — auto-detect a repo-root `DESIGN.md` (recommended) or
   require `--design=`?
4. **Non-React verification** — extend `verify_integration.py` per target, or
   ship a documented manual check first?
5. **Adapter-or-project policy** — when a `## Target:` adapter is *absent*, always
   agent-project, or warn/decline for stateful components without an adapter?

## Next step

This is forward-looking — it depends on COMPONENT.md and the DESIGN.md adapter
existing. On approval, hold it as roadmap PR 7+ and convert to an execution plan
once Workstreams A and B land. Resolve the five decisions above before drafting
the plan.
