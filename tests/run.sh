...
if [ -f "$UTIL_DIR/utilities.tokens.json" ]; then
  ...
  # Bundle lives at $UTIL_DIR/utilities.css; per-family partials live flat in
  # $UTIL_DIR/<family>.css. The generator writes bundle to $UTIL_REGEN_DIR/utilities.css
  # and per-family to $UTIL_REGEN_DIR/utilities/<family>.css, so we compare accordingly.
  IDEMPOTENT=1
  if ! diff -q "$UTIL_DIR/utilities.css" "$UTIL_REGEN_DIR/utilities.css" >/dev/null; then
    IDEMPOTENT=0
  fi
  for regen_file in "$UTIL_REGEN_DIR/utilities/"*.css; do
    [ -f "$regen_file" ] || continue
    fname="$(basename "$regen_file")"
    if ! diff -q "$UTIL_DIR/$fname" "$regen_file" >/dev/null 2>&1; then
      IDEMPOTENT=0
    fi
  done
